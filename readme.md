...
| 2 | Currency | On | Quest-class items *not* tied to an active quest — reputation/faction tokens like Mark of Thrallmar, used like a stackable currency. |
| 3 | BoE | On | Unbound items whose bind type is Bind on Equip. |
| 4 | Soulbound *(parent)* | On | Bound gear and other bound items. Expands into three children — see below. |
| 5 | Potions & Consumables | On | Consumable-class items other than food/drink. |
| 6 | Food & Water | On | The Food & Drink consumable subclass specifically. |
| 7 | Reagents | On | Reagent-class items. |
| 8 | Gems | On | Gem-class items. |
...
| — | Miscellaneous | Always on | The permanent catch-all. Everything unmatched lands here, as does any category folded shut all the way to the root. |

### Soulbound's children

Soulbound is the one top-level category with permanent sub-structure:

- **Equipment** — bound gear and relics. This is also where ItemRack equipment-set subcategories attach — see [ItemRack equipment sets](#itemrack-equipment-sets).
...
`Cloth` `Leather` `Metal & Stone` `Herb` `Meat` `Elemental` `Enchanting` `Jewelcrafting` `Miscellaneous`
...
Anything reported under a subtype that isn't in these lists still gets grouped — under that parent's own Miscellaneous child — instead of being lost.
...
Example: with Flying and Mounts both unchecked but Soulbound itself checked, a flying mount lands in the plain Soulbound bucket. If Soulbound were unchecked too, it would fall all the way to Miscellaneous instead.
...
Below the top-level options sits the full category list, in whatever order it's currently sorted. Three things happen here:

- **Enable/disable** — every category is a checkbox; unchecking one applies the fallback rule described above.
- **Reorder** — drag one category onto another to swap their positions. This works at every level: the root list, and again inside each category that has its own sub-panel (Soulbound, Trade Goods, Recipes). Order is shared between bag and bank.
- **Drill into sub-panels** — Soulbound, Trade Goods, and Recipes each expand into their own nested settings group (with their own Enable toggle at the top and their own drag list below) rather than showing every subcategory flattened into one long list.
